...
{
    // TODO
    // Move to the spot in the file where the student's data should be
    if ((lseek(fd, (off_t)id * STUDENT_RECORD_SIZE, SEEK_SET)) < 0) {
        return ERR_DB_FILE;
    }

    // Try to read the student's data into the provided structure
    // If we can't read the full record or if the ID is 0, student not found
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    student_t existing;
    if (get_student(fd, id, &existing) == NO_ERROR) {
        printf(M_ERR_DB_ADD_DUP, id); // Student exists, so we can't add them again
        return ERR_DB_OP;
    }
    ...
    printf(M_STD_ADDED, id); // Confirm student was added scuessfully
    return NO_ERROR;
}
...
int del_student(int fd, int id)
{
    ...
    student_t temp;
    int rc = get_student(fd, id, &temp);
    if (rc == SRCH_NOT_FOUND) { // Student not found in the database
        printf(M_STD_NOT_FND_MSG, id);
        return ERR_DB_OP;
    } else if (rc == ERR_DB_FILE) { // Error reading from the database
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    // Move to the student's position and overwrite their data with an empty record
    if ((lseek(fd, (off_t)id * STUDENT_RECORD_SIZE, SEEK_SET) < 0) || (write(fd, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE)) {
        printf(M_ERR_DB_WRITE); // Error writing to the database
        return ERR_DB_FILE;
    }

    printf(M_STD_DEL_MSG, id); // Confirm student was deleted successfully
    return NO_ERROR;
}
...
int count_db_records(int fd)
{
    ...
    if (lseek(fd, 0, SEEK_SET) < 0) {
        printf(M_ERR_DB_READ); // Error if we can't move to the start
        return ERR_DB_FILE;
    }

    int record_count = 0; // To count the number of records
    student_t student; // Temporary storage for each record
    ...
    if (record_count == 0) {
        printf(M_DB_EMPTY); // No records found
    }
    else {
        printf(M_DB_RECORD_CNT, record_count); // Records found
    }

    return record_count; // Return the total count
}
...
int print_db(int fd)
{
    ...
    int printed_count = 0; // To track how many records we've printed
    student_t student; // Temporray storage for each record

    // Read and print each student record
    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        // Skip empty records
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, sizeof(student)) == 0) {
            continue;
        ...
        // Print the header only once, before the firts valid record
        if (printed_count++ == 0) {
            printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST_NAME", "LAST_NAME", "GPA");
        }

        // Print the student details
        printf(STUDENT_PRINT_FMT_STRING, student.id, student.fname, student.lname, student.gpa / 100.0f);
    }

    // If no records werre printed, the database is empty
    if (printed_count == 0) {
        printf(M_DB_EMPTY);
    }
...
{
    // TODO
    // Check if the student pointer is NULL or if the student ID is 0 (invalid student)
    if (!s || s->id == 0) {
...
{
    // TODO
    // Move to the start of the original database file
    if (lseek(fd, 0, SEEK_SET) < 0) {
        printf(M_ERR_DB_READ); // Error if we can't move to the start
        return ERR_DB_FILE;
    }

    // Open a temporary database file to store the compressed data
    int tmp_fd = open_db(TMP_DB_FILE, true);
    if (tmp_fd < 0) {
        printf(M_ERR_DB_OPEN); // Error opening the temporarry file
        return ERR_DB_FILE;
    }

    student_t student;
    int count = 1; // Track the new record position in the compressed file

    // Read each student record from the original database
    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        // Copy only non-empty (valid) student records
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, sizeof(student)) != 0) {
            // Move to the correct spot in the temp file and write the student record
            if (lseek(tmp_fd, (off_t)count++ * STUDENT_RECORD_SIZE, SEEK_SET) < 0 ||
    ...
    // Close both the original and temporary database files
    close(fd);
    close(tmp_fd);

    // Replace the original database with the compressed noe
    if (rename(TMP_DB_FILE, DB_FILE) < 0) {
        printf(M_ERR_DB_CREATE); // Error renaming the file
        return ERR_DB_FILE;
    }

    // Reopen the compressed database file for further use
    int new_fd = open_db(DB_FILE, false);
    if (new_fd < 0) {
        printf(M_ERR_DB_OPEN); // Error reopening the database
        return ERR_DB_FILE;
    }

    printf(M_DB_COMPRESSED_OK); // Confirm the compression was successful
    return new_fd;
}
